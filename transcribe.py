...
RECORD_SECONDS = 5
FINALS = []
from flowing_conversation import *
...
def on_message(self, msg):
    ...
    if "results" in data:
        if data["results"][0]["final"]:
            ...
            print("final")
            print(data)
            # todo, send text in here
            final_transcript = data['results'][0]['alternatives'][0]['transcript']
            confidence = data['results'][0]['alternatives'][0]['confidence']
            print(confidence)
            if confidence >= 0.7:
                FINALS.append(final_transcript)
